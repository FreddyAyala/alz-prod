...
resource "azurerm_public_ip" "lz-pip-bastion" {
  ...
  resource_group_name = values(values(module.enterprise_scale[0].azurerm_firewall)[0])[0].resource_group_name
...
resource "azurerm_bastion_host" "lz-bastion-host" {
  ...
  ip_configuration {
    name                 = "configuration"
    subnet_id            = values(values(module.enterprise_scale[0].azurerm_subnet.connectivity)[0])[4]
    public_ip_address_id = azurerm_public_ip.lz-pip-bastion.id
  }

  depends_on = [module.enterprise_scale[0]]
  provider   = azurerm.connectivity
}
